...
function VuelosByDestinoPage() {
  ...
  if (!destino) {
    return (
      <div className="flex flex-col justify-center items-center h-screen text-white px-4 text-center">
        <p className="text-2xl font-semibold mb-4">
          Especifique un destino para buscar vuelos.
        </p>
        <p className="text-lg text-gray-400">xd </p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="flex justify-center items-center h-screen">
        <p className="text-xl text-red-500">Error al cargar vuelos: {error}</p>
      </div>
    );
  }
  if (noResults) {
  ...
  if (vuelos.length === 0 && !loading && !error && !noResults) {
    return (
      <div className="flex justify-center items-center h-screen">
        <p className="text-xl text-gray-400">
          No hay vuelos disponibles para {destino} en este momento.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-white mb-6 text-center">
        Vuelos disponibles hacia:{" "}
        <span className="text-emerald-400">{destino}</span>
      </h1>
          ...
          >
            <div>
              <h2 className="text-xl font-semibold mb-2">
                {vuelo.origen}{" "}
                <span className="text-gray-400 text-sm font-normal">hacia</span>{" "}
                {vuelo.destino}
              </h2>
